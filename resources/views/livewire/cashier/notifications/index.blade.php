...
<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            <p class="text-sm text-zinc-500">Critical</p>
            <p class="mt-2 text-2xl font-semibold">
                {{ collect($alerts)->where('severity', 'danger')->count() }}
            </p>
        </div>

        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            <p class="text-sm text-zinc-500">Warning</p>
            <p class="mt-2 text-2xl font-semibold">
                {{ collect($alerts)->where('severity', 'warning')->count() }}
            </p>
        </div>
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p class="mt-2 text-2xl font-semibold">
                {{ collect($alerts)->where('severity', 'info')->count() }}
            </p>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">
